              ...
              getVoteByRequest(page, (retCode, retMsg) {
                if (retCode == 0) {
                  if (retMsg.isNotEmpty) {
                    ...
                    // 如果返回空字符串，说明是缓存的页面，直接调用callback
                    setState(() {
                      callback();
                    });
                  }
  ...
  Future<void> getVoteByRequest(int page, VoteCallBack callBack) async {
    ...
    new HttpUtils().httpGet(requestUrl,
      ...
      if (code == 0) {
        ...
        if (response.retcode == 0) {
          callBack(response.retcode!, response.data!);
        } else {
          callBack(response.retcode!, response.retmsg!);
        }
      } else {
        callBack(code, retmsg);
      }
    });
  }
}
